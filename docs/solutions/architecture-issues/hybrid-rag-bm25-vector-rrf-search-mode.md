...
- `CatalogQueryRequest.match_threshold: float = Field(default=0.02, ge=0.0, le=1.0)` — canonical API field for the per-request score threshold; filters RRF noise band post-merge. Passed to `CatalogRetrievalService.query()` as `match_threshold`. Legacy `min_score` is accepted as an API input alias for compatibility.
...
- Server-side default: `InvoiceConfig.rag_match_threshold` (default `0.0`) sets the service-level floor independently of the request override; the explicit request value takes priority when provided
...
- Default to `hybrid` — it handles both fuzzy NL and exact-code queries. Add a query threshold (default `0.02`) to filter RRF noise; small catalogs compress score ranges and need a floor.
