...
class SupabaseInvoiceImportRepository(InvoiceImportRepository):
    ...
    def _product_sync_input_payload(self, data: ProductSyncRecordInput) -> dict[str, Any]:
        return {
            ...
            "p_embedding_model": data.embedding_model,
            "p_name": data.name,
            "p_barcode": data.barcode,
            "p_category": data.category,
            "p_uom": data.uom,
            "p_supplier": data.supplier,
            "p_price_eur": data.price_eur,
            "p_price_50": data.price_50,
            "p_price_70": data.price_70,
            "p_price_100": data.price_100,
            "p_markup": data.markup,
            "p_source_import_id": data.source_import_id,
            "p_source_row_id": data.source_row_id,
            "p_invoice_number": data.invoice_number,
            "p_sync_status": data.sync_status,
            "p_attempt_count": data.attempt_count,
            "p_last_error": data.last_error,
            "p_claimed_at": data.claimed_at.isoformat() if data.claimed_at else None,
            "p_claimed_by": data.claimed_by,
            "p_next_retry_at": data.next_retry_at.isoformat() if data.next_retry_at else None,
            "p_last_synced_at": data.last_synced_at.isoformat() if data.last_synced_at else None,
        }
